...
impl Interpreter {
    ...
    pub fn new(
        ...
        // Map transaction parameters to their corresponding DUT pin values
        // For INPUT parameters only - output parameters are populated later when observed
        // For example: parameter 'a' -> value of 'DUT.a' from trace
        ...
        for arg in &transaction.args {
            ...
            // For input parameters, fetch their symbol_id & their names
            let param_symbol_id = arg.symbol();
            let param_name = symbol_table[param_symbol_id].name();

            // For each input parameter, find the corresponding DUT pin
            // (e.g. input param "a" corresponds to the DUT pin "DUT.a")
    ...
    pub fn evaluate_stmt(
        ...
        match stmt {
            ...
            Stmt::AssertEq(lhs_expr, rhs_expr) => {
                match (
                    self.evaluate_expr(lhs_expr, ctx),
                    self.evaluate_expr(rhs_expr, ctx),
                ) {
                    // `lhs_expr` is unknown (e.g. an output parameter)
                    // but `rhs_expr` evaluates to a concrete `value`
                    (Err(_), Ok(ExprValue::Concrete(value))) => {
                        // If `lhs_expr` is an identifier `lhs_symbol_id`,
                        // extend the args_mapping so that `lhs_symbol_id |-> value`
                        if let Expr::Sym(lhs_symbol_id) = &self.transaction[lhs_expr] {
                            let lhs_name =
                                self.symbol_table[lhs_symbol_id].full_name(&self.symbol_table);
                            info!(
                                "{} is not in args_mapping yet, but {} evaluates to {}, adding to args_mapping",
                                lhs_name,
                                self.format_expr(rhs_expr),
                                serialize_bitvec(&value, ctx.display_hex)
                            );
                            self.args_mapping.insert(*lhs_symbol_id, value);
                        } else {
                            info!("{} is ???", self.format_expr(lhs_expr));
                        }
                    }
                    // `rhs_expr` is unknown (e.g. an output parameter)
                    // but `lhs_expr` evaluates to a concrete `value`
                    (Ok(ExprValue::Concrete(value)), Err(_)) => {
                        // If `rhs_expr` is an identifier `rhs_symbol_id`,
                        // extend the args_mapping so that `rhs_symbol_id |-> value`
                        if let Expr::Sym(rhs_symbol_id) = &self.transaction[rhs_expr] {
                            let rhs_name =
                            ...
                            self.args_mapping.insert(*rhs_symbol_id, value);
                        } else {
                            info!("{} is ???", self.format_expr(rhs_expr));
                        }
                    }
                    // Both arguments to `assert_eq` are unknown
                    (Err(_), Err(_)) => {
                        info!("{} is ???", self.format_expr(lhs_expr));
                        info!("{} is ???", self.format_expr(rhs_expr));
                    }
                    // If one argument is `DontCare`, we log that
                    // the other argument is ???
                    (Err(_), Ok(ExprValue::DontCare)) => {
                        info!("{} is ???", self.format_expr(lhs_expr));
                    }
                    (Ok(ExprValue::DontCare), Err(_)) => {
                        info!("{} is ???", self.format_expr(rhs_expr));
                    }
                    // Both arguments to `assert_eq` successfully evaluate
                    // to values (this is the normal case)
                    (Ok(_), Ok(_)) => {
                        // In the monitor, we don't evaluate assertions since
                        // we're inferring values for input/output parameters,
