                        ...
                        match clock_signal_parts.last() {
                            Some(var_name) => {
                                    ...
                                    .find(|v| hierachy[*v].name(hierachy) == *var_name)
                                {
                                    ...
                                    // (We exclude the final element from `clock_signal_parts`,
                                    // since that is equal to `var_name`)
                                    match hierachy.lookup_var(
                                        &clock_signal_parts[..clock_signal_parts.len() - 1],
                                        var_name,
                                    ) {
                                        Some(var_ref) => {
    ...
    fn step(&mut self) -> StepResult {
        ...
        match self.sampling_mode {
            ...
            WaveSamplingMode::RisingEdge(clock_signal_ref) => {
                ...
                let old_time = self.format_time(self.time_step);
                ...
                while self.time_step < total_steps {
                    ...
                    if prev_value == "0" && new_value == "1" {
                        let new_time = self.format_time(self.time_step);
                        info!("Advanced clock time from {old_time} -> {new_time}");
