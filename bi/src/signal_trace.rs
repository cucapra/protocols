...
impl SignalTrace for WaveSignalTrace {
    fn step(&mut self) -> StepResult {
        if self.step as usize == self.wave.time_table().len() - 1 {
            StepResult::Done
        } else {
            debug_assert!((self.step as usize) < self.wave.time_table().len() - 1);
            self.step += 1;
            StepResult::Ok
        }
    }

    fn get(&self, instance_id: u32, pin_id: SymbolId) -> BitVecValue {
        let key = PortKey {
            instance_id,
            pin_id,
        };
        let signale_ref = self.port_map[&key];
        let signal = self
            .wave
            .get_signal(signale_ref)
